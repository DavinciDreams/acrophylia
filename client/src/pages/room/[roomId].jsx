...
const GameRoom = () => {
  ...
  const [isStarting, setIsStarting] = useState(false); // Debounce state
  ...
  const startGame = useCallback(
    debounce(() => {
      if (roomId && isCreator && !isStarting) {
        ...
        setTimeout(() => setIsStarting(false), 1000); // Reset after 1s
      }
    }, 500),
  ...
  const resetGame = () => {
    if (isCreator && roomId) {
      ...
      socket.emit('resetGame', roomId); // New server event
      setRoundNum(0);
